...
const C={ink:'#30243d',deep:'#17172a',night:'#24213d',cream:'#fff2d1',gold:'#f3c665',pink:'#f49ab8',wood2:'#71483c'};
...
let yuli={x:96,y:105,dir:'down',frame:0,state:'counter'};
...
function drawSprite(img,x,y,dir='down',frame=0,targetW=34){if(!img.complete||!img.naturalWidth)return;const fw=img.naturalWidth/3,fh=img.naturalHeight/4;const col=frame===0?0:1+((frame-1)%2),row=rowByDir[dir]??0,targetH=targetW*(fh/fw);ctx.drawImage(img,col*fw,row*fh,fw,fh,Math.round(x-targetW/2),Math.round(y-targetH),Math.round(targetW),Math.round(targetH));}
...
function drawWoodWall(){
  px(0,0,W,88,'#6b503c');
  for(let x=0;x<W;x+=4){px(x,0,2,88,(x/4)%2?'#806047':'#574132');px(x+2,0,1,88,'#9a7657');}
  px(0,0,W,10,'#262326');px(0,82,W,6,'#2b2728');
  for(let x=24;x<=150;x+=42){px(x,12,18,3,'#f8dfb8');px(x+3,15,12,2,'#fff4d6');}
  text('PilaGirl',61,38,9,'#f6a6c6','center');
  text('GYM',119,43,8,'#fff7e9','center');
}
function drawFloor(){
  px(0,88,W,H-88,'#eadfce');
  const tile=48;
  for(let y=88;y<H;y+=tile){for(let x=0;x<W;x+=tile){const alt=((x/tile)+Math.floor((y-88)/tile))%2;px(x,y,tile,tile,alt?'#eadfce':'#e4d6c2');px(x,y,tile,2,'#c7b9a5');px(x,y,2,tile,'#c7b9a5');px(x+3,y+3,tile-6,1,'#f5ecdf');}}
}
function drawReceptionDesk(){
  // warm glow
  px(38,171,116,4,'#f6d79c');px(43,175,106,2,'#fff0c5');
  // rounded silver body
  px(40,122,112,48,'#a7a6a3');
  px(34,129,8,34,'#a7a6a3');px(150,129,8,34,'#a7a6a3');
  px(36,125,120,8,'#d8d5d0');px(42,121,108,7,'#eeeae4');
  px(42,132,108,34,'#bdbbb7');
  // curved end pixels
  px(34,133,4,25,'#b7b5b1');px(38,126,5,38,'#c8c5c0');
  px(154,133,4,25,'#989794');px(149,126,5,38,'#b6b4b0');
  // brushed metal lines
  for(let x=45;x<149;x+=13){px(x,135,1,27,x%26?'#d4d1cc':'#8f8e8b');}
  px(40,165,112,4,'#777572');px(44,166,104,2,'#ded9d1');
  // equipment on top
  rect(50,110,20,12,'#303337','#26232b',1);px(53,113,14,5,'#686d72');px(57,122,5,4,'#2b2b2d');
  rect(127,109,17,14,'#d9d4cc','#3c3638',1);for(let r=0;r<2;r++)for(let c=0;c<2;c++)px(130+c*7,112+r*5,5,3,'#77797b');
}
function drawGymWorld(){ctx.save();drawFloor();drawWoodWall();drawYuli(yuli.x,yuli.y,yuli.dir,yuli.frame,38);drawReceptionDesk();rect(75,307,42,29,'#514b47',C.ink,3);px(80,312,32,24,'#2e2b29');drawBoyfriend(player.x,player.y,player.dir,player.frame,36);ctx.restore();text('02.16 · 헬스보이짐',9,7,7,C.cream);}
...
function enterGym(){scene='gym';cameraX=0;player={x:96,y:300,dir:'up',frame:0};yuli={x:96,y:121,dir:'down',frame:0,state:'counter'};foundYuli=false;actionButton.classList.add('hidden');showDialogue(['308호의 첫 기억이 열렸다.','2월 16일, 헬스보이짐.']);}
...
function update(dt){
  ...
  const oldX=player.x,oldY=player.y;const vx=joy.dx/len,vy=joy.dy/len,speed=72*dt;
  ...
  if(scene==='gym'){
    player.x=Math.max(12,Math.min(180,player.x));player.y=Math.max(92,Math.min(309,player.y));
    // Only the silver body blocks movement. Wide side paths stay open.
    const hitX=player.x>31&&player.x<161;
    const hitY=player.y>118&&player.y<176;
    if(hitX&&hitY){
      if(oldY>=176)player.y=176;
      else if(oldY<=118)player.y=118;
      else if(oldX<=31)player.x=31;
      else if(oldX>=161)player.x=161;
      else {player.x=oldX;player.y=oldY;}
    }
    const nearYuli=Math.hypot(player.x-yuli.x,player.y-yuli.y)<33;foundYuli=nearYuli;actionButton.classList.toggle('hidden',!nearYuli);
  }
}
