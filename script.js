...
const C={ink:'#30243d',deep:'#17172a',night:'#24213d',cream:'#fff2d1',gold:'#f3c665',pink:'#f49ab8',rose:'#cf6489',lav:'#c7b0e8',skin:'#f4c5aa',skin2:'#df9f86',hair:'#3b3139',wood:'#9b674f',wood2:'#71483c',gym:'#747b80',gym2:'#5f666b',white:'#fffaf0',black:'#17131d',blue:'#7db0d1'};
...
let scene='title',elapsed=0,cameraX=0,lastMoveTime=0;
...
function drawSprite(img,x,y,dir='down',frame=0,targetW=34){
  ...
  const col=frame===0?0:1+((frame-1)%2);
  const row=rowByDir[dir]??0;
  const targetH=targetW*(fh/fw);
...
function drawNpc(x,y,shirt='#c77c5f'){
  px(x-7,y-25,14,10,C.hair);px(x-6,y-18,12,9,C.skin);px(x-5,y-8,10,10,shirt);px(x-5,y+2,4,8,C.black);px(x+1,y+2,4,8,C.black);px(x-5,y+10,5,2,C.white);px(x+1,y+10,5,2,C.white);
}
...
function drawTitle(){
  ...
  drawBoyfriend(72,270,'down',0,40);drawYuli(121,270,'down',0,40);
  text('기억 속 방들을 찾아가 보세요',96,294,7,C.cream,'center');
}
function drawLobby(){
...
function drawGymWorld(){
  ctx.save();ctx.translate(-cameraX,0);px(0,0,WORLD_W,H,'#d8c1aa');px(0,0,WORLD_W,82,'#8c6d62');
  ...
  drawVending(18,45);drawWater(22,108);drawSofa(18,164);drawCounter(83,55);rect(7,16,69,22,'#5c4037',C.ink,2);text('HEALTH BOY GYM',41,23,6,C.cream,'center');px(180,143,10,3,C.gold);px(187,139,3,11,C.gold);
  if(['counter','walking','exercise'].includes(yuli.state))drawYuli(yuli.x,yuli.y,yuli.dir,yuli.frame,36);
  drawDumbbells(210,70);drawSmith(288,52);drawMachine(334,45,1);drawMachine(350,92,2);rect(212,151,112,74,'#40464a',C.ink,2);drawMachine(230,172,0);drawMachine(275,174,1);rect(327,145,45,61,'#72756b',C.ink,2);px(334,155,8,34,'#ad82c8');px(347,155,8,34,'#d08191');px(360,155,8,34,'#7199bd');drawTreadmill(212,247);drawTreadmill(244,247);drawTreadmill(276,247);drawMachine(335,232,2);
  [{x:230,y:104,s:'#c77c5f'},{x:305,y:105,s:'#7b9d7b'},{x:227,y:273,s:'#9274a4'},{x:294,y:272,s:'#c69b58'},{x:355,y:260,s:'#6d9bc0'}].forEach(n=>drawNpc(n.x,n.y,n.s));
  drawBoyfriend(player.x,player.y,player.dir,player.frame,36);ctx.restore();text('02.16 · 헬스보이짐',9,7,7,C.cream);
}
...
function enterGym(){scene='gym';cameraX=0;player={x:110,y:257,dir:'up',frame:0};yuli={x:125,y:108,dir:'down',frame:0,state:'counter'};foundYuli=false;actionButton.classList.add('hidden');showDialogue(['308호의 첫 기억이 열렸다.','2월 16일, 헬스보이짐.'],startGymIntro);}
function startGymIntro(){showHint('카운터의 율리가 운동 공간으로 들어간다…',1800);setTimeout(()=>{yuli.state='walking';yuli.dir='right';yuli.frame=1;},400);}
...
function update(dt){
  if(scene==='gym'&&yuli.state==='walking'){yuli.x+=42*dt;yuli.frame=1+Math.floor(elapsed/180)%2;if(yuli.x>315){yuli.x=342;yuli.y=174;yuli.dir='down';yuli.frame=0;yuli.state='exercise';showHint('어디로 갔지? 오른쪽으로 따라가 보자!',1800);}}
  if(!joy.active||!dialogue.classList.contains('hidden'))return;const len=Math.hypot(joy.dx,joy.dy);if(len<8){player.frame=0;return;}const vx=joy.dx/len,vy=joy.dy/len,speed=72*dt;player.x+=vx*speed;player.y+=vy*speed;player.frame=1+Math.floor(elapsed/150)%2;if(Math.abs(vx)>Math.abs(vy))player.dir=vx>0?'right':'left';else player.dir=vy>0?'down':'up';
  ...
  if(scene==='gym'){player.x=Math.max(12,Math.min(WORLD_W-12,player.x));player.y=Math.max(102,Math.min(309,player.y));cameraX=Math.max(0,Math.min(WORLD_W-W,player.x-W/2));const nearYuli=yuli.state==='exercise'&&Math.hypot(player.x-yuli.x,player.y-yuli.y)<32;foundYuli=nearYuli;actionButton.classList.toggle('hidden',!nearYuli);}
}
...
joystick.addEventListener('pointerdown',e=>{e.preventDefault();joy.active=true;joy.id=e.pointerId;joystick.setPointerCapture(e.pointerId);joyMove(e);});joystick.addEventListener('pointermove',e=>{if(joy.active&&e.pointerId===joy.id)joyMove(e);});function joyEnd(e){if(joy.id!==null&&e.pointerId!==joy.id)return;joy.active=false;joy.id=null;joy.dx=joy.dy=0;player.frame=0;joystickKnob.style.transform='translate(0,0)';}joystick.addEventListener('pointerup',joyEnd);joystick.addEventListener('pointercancel',joyEnd);
startButton.addEventListener('click',enterLobby);startButton.addEventListener('pointerup',e=>{e.preventDefault();if(scene==='title')enterLobby();});actionButton.addEventListener('click',tryAction);window.addEventListener('keydown',e=>{const map={ArrowLeft:[-35,0],ArrowRight:[35,0],ArrowUp:[0,-35],ArrowDown:[0,35]};if(map[e.key]){joy.active=true;[joy.dx,joy.dy]=map[e.key];}if(e.key==='Enter'||e.key===' ')tryAction();});window.addEventListener('keyup',()=>{joy.active=false;joy.dx=joy.dy=0;player.frame=0;});
