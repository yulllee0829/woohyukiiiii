...
(function(){
  ...
  drawGymSideRoom=function(){if(active){drawPharmacy();return;}previousDrawSide();};

  function nearPharmacist(){return active&&introDone&&!popupOpen&&!completed&&Math.hypot(player.x-96,player.y-184)<58;}

  const previousUpdate=update;
  update=function(dt){
    if(!active){previousUpdate(dt);return;}
    if(!introDone||talking||popupOpen||completed||!dialogueEl.classList.contains('hidden')||!inventoryPanel.classList.contains('hidden')){player.frame=0;return;}
    const len=Math.hypot(joy.dx,joy.dy);
    if(!joy.active||len<8){player.frame=0;}
    else{
      const vx=joy.dx/len,vy=joy.dy/len;
      player.x+=vx*72*dt;player.y+=vy*72*dt;
      player.x=Math.max(14,Math.min(178,player.x));player.y=Math.max(178,Math.min(322,player.y));
  ...
  function setMission3(text,bang=true){mission.textContent=text;mission.classList.remove('hidden','mission-bang');if(bang){void mission.offsetWidth;mission.classList.add('mission-bang');}}

  function enterPharmacy(){
    if(active||entering)return;
    entering=true;active=true;completed=false;introDone=false;talking=false;popupOpen=false;selected.clear();viewedMedicine=null;
    scene='gymSide';player={x:96,y:322,dir:'up',frame:0};
    inventory=[];selectedItem=null;window.gymPickupHelpShown=true;renderInventory();
    mission.classList.add('hidden');inventoryPanel.classList.add('hidden');pocket.classList.remove('hidden');action.classList.add('hidden');joystickEl.classList.add('hidden');if(fade)fade.classList.remove('active');
    setTimeout(()=>showDialogue(['우혁 : 머리가 좀 아프네ㅜㅜ 약을 사야겠어!'],()=>{introDone=true;joystickEl.classList.remove('hidden');setMission3('미션 3 : 약 구매하기',true);}),650);
  }

  if(fade)new MutationObserver(()=>{if(!active&&!entering&&fade.classList.contains('active'))setTimeout(enterPharmacy,2250);}).observe(fade,{attributes:true,attributeFilter:['class']});
  ...
  function openMedicinePopup(){
    popupOpen=true;selected.clear();viewedMedicine=null;joystickEl.classList.add('hidden');action.classList.add('hidden');
    ...
    panel.querySelectorAll('.medicine-card').forEach(card=>{
      const key=card.dataset.med;
      card.querySelector('.medicine-info').addEventListener('click',()=>{
        viewedMedicine=key;
        panel.querySelectorAll('.medicine-card').forEach(c=>c.classList.toggle('viewing',c.dataset.med===key));
        desc.textContent=medicineDescription(key);
      });
      card.querySelector('.medicine-check').addEventListener('click',event=>{
        event.stopPropagation();
        if(selected.has(key))selected.delete(key);else selected.add(key);
        card.classList.toggle('selected',selected.has(key));
        card.querySelector('.medicine-check').textContent=selected.has(key)?'✓':'';
  ...
  function missionClear(){setMission3('미션 3 CLEAR!',true);mission.classList.add('mission-clear');joystickEl.classList.add('hidden');action.classList.add('hidden');setTimeout(()=>{if(fade)fade.classList.add('active');},1250);}
  ...
  function finishMedicineChoice(){
    ...
    if(selected.size===2){
      showDialogue(['약사 : 사랑꾼이시네요ㅎㅎ','약사 : 꼭 결혼하세요~','약사 : 말차 초코파이 드세요ㅎㅎ!!'],()=>{
        addChocoToBag();
        showDialogue(['시스템 : 말차 초코파이를 받았다!'],()=>{talking=false;completed=true;missionClear();});
      });
  ...
  function talkToPharmacist(){
    ...
    showDialogue(['우혁 : 머리가 아파서요ㅜㅜ','약사 : 이 두 가지 약이 좋으실 것 같은데, 한번 보시겠어요?'],()=>{talking=false;openMedicinePopup();});
  }

  document.addEventListener('click',event=>{if(!active||event.target!==action)return;if(nearPharmacist()){event.preventDefault();event.stopImmediatePropagation();talkToPharmacist();}},true);

  function enforcePharmacyUi(){
    if(active){
      pocket.classList.remove('hidden');
      const talk=nearPharmacist()&&!talking&&!popupOpen&&!completed;
      if(talk)action.classList.remove('hidden');action.classList.toggle('action-talk',talk);action.classList.toggle('action-hand',false);action.setAttribute('aria-label','약사에게 말 걸기');
      if(!dialogueEl.classList.contains('hidden')&&dialogueEl.classList.contains('speaker-hong')){
        dialogueEl.style.setProperty('left','12px','important');
        dialogueEl.style.setProperty('top','14%','important');
  ...
  style.textContent=`
    ...
    .medicine-card{position:relative;min-width:0;border:2px solid #998c86;border-radius:10px;background:#fff;color:#2b2138;overflow:hidden}
    .medicine-card.viewing{border-color:#6f94a7;box-shadow:0 0 0 2px rgba(111,148,167,.16)}
    .medicine-card.selected{background:#fff7fb}
    .medicine-info{appearance:none;pointer-events:auto;width:100%;min-width:0;padding:10px 5px 8px;border:0;background:transparent;color:#2b2138;font-weight:700;font-size:12.5px;line-height:1.25}
    .medicine-info img{display:block;width:78px;height:72px;object-fit:contain;margin:0 auto 5px;image-rendering:pixelated}
    .medicine-check{appearance:none;position:absolute;z-index:2;left:7px;top:7px;width:23px;height:23px;border:2px solid #8c7e78;border-radius:50%;background:#fff;color:#c64270;font-size:15px;font-weight:900;line-height:18px;padding:0;pointer-events:auto}
    .medicine-card.selected .medicine-check{border-color:#d55883;background:#ffe1ec}
